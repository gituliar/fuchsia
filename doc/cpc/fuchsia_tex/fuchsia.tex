...
\usepackage{amssymb,amsmath,color,enumitem,hyperref,xspace,fancyvrb}
...
\author{Oleksandr Gituliar\corref{cor1}}
\ead{oleksandr@gituliar.net}
...
{\em Additional comments including Restrictions and Unusual features:}\\
    ...
    Performance depends upon the input matrix, it's size, number of singular points and their degrees.
    It takes around an hour to reduce an example $74\times74$ matrix with 20 singular points on a PC with a 1.7GHz Intel Core i5 CPU.
...
Next, source file \code{fuchsia.py} should be downloaded from the website \url{https://github.com/gituliar/fuchsia} (on the same website you will also find multiple examples of matrices before and after reduction, as well as the up-to-date documentation for \fuchsia).
...
\fuchsia is open-source in it's nature and depends on free software tools only: the programming language \python and the computer algebra system \maximasage, which makes it available for everyone.
