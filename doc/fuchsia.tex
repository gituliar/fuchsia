...
To run \fuchsia use the command\footnote{For brevity, sometimes we use a shortcut \code{fuchsia} which is equal to \code{sage -python fuchsia.py}.}
\begin{Verbatim}
...
In spite of that we still have difficulties to reduce somewhat smaller matrices with complex singular points due to the lack of support of factorization of polynomials with complex (or, in the general case, algebraic) roots in the \maximasage system, which we hope to improve in the next version of the program.
Another promising direction for improvements could be a full support for multivariate polynomials and symbolic singular points, which would allow to reduce differential equations for multi-scale Feynman master integrals.
In fact, these two issues are tightly connected.
