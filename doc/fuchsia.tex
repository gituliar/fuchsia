...
Systems in such a form can be trivially solved, which makes the construction of the transformation $\M T(x,\eps)$ crucial for obtaining solutions of the initial equations.
...
In Section~\ref{sec:4} we make a summary and discuss limitations and possible extensions of the current implementation.
...
Though it helps a lot in some situation, \maple can not deal with huge intermediate expressions which arise during the reduction process of some advanced examples.
On the one hand, it is partly connected with non-optimal algorithmic optimizations of \fuchsia code.
On the other hand, it happens due to poor implementation of factorization of polynomials with algebraic (or even complex) roots in the \maxima and \maple.
In fact, these two issues are tightly interconnected and we hope to make discussed optimizations in the next version of the program.

Despite of the discussed limitation \fuchsia can show great performance in some cases.
...
Moreover, \fuchsia can deal with symbolic singular points.
This allows to use it for reduction of single-variable differential equations for multi-scale Feynman master integrals.
