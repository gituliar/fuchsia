...
An easy way to use \fuchsia is to run the command \code{fuchsia}\footnote{If you have used \code{sage -pip} to install \fuchsia, then you will find this script at \code{<path~to~SageMath>/local/bin/fuchsia}.} or \code{sage -python fuchsia.py} which has the following invocation signature:
