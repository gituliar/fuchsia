...
\definecolor{fuchsia}{RGB}{123,31,162}
...
Let us change the basis of eq.~\eqref{eq:dj} from $\V J$ to $\V J'$ by using transformation $\M T(x,\eps)$:
\begin{equation}
  \V J = \M T(x,\eps)\, \V J'.
\end{equation}
...
In general case transformations $\M T(x,\eps)$ may have arbitrary form, however in the scope of this paper we'll be using transformation matrices rational in both $x$ and $\eps$.
...
In principle, Moser orders of equivalent systems are not the same, i.e., $m_k(\M A) \ne m_k(\M A')$.
For that reason, in order to make our analysis of singularities complete, we introduce the {\em minimum order} of $\M A(x,\eps)$ at $x=x_k$:
\begin{equation}
\label{eq:muk}
...
In the case of a single ODE of order~$n$, the minimum Moser order $\mu_k$ can be computed explicitly from the power analysis of its coefficients (for more details see generalization of the Fuchs' theorem~\cite{Mos59}).
This is not possible for ODE systems like~\eqref{eq:dj}; instead we have a critetion by which we can tell if a system is Moser-reducible or not (see condition~\eqref{eq:red_cond}), and if so, we also have a way to constructs a transformation that reduces $rank(\M A)_{k0}$ by at least $1$.

So, to determine $\mu_k(\M A)$ we must use a {\em recursive criterion} which consist of these steps:
\begin{enumerate}
    \item Check if a matrix $\M A(x,\eps)$ is reducible at $x=x_k$. If not, stop.
    \item Find a transformation $\M T(x,\eps)$ that decreases $rank(\M A_{k0})$ and thus $m_k(\M A)$.
    \item Apply $\M T(x,\eps)$ and repeat from Step 1.
\end{enumerate}

We describe these steps in more details in the following section, what we would like to emphasize here is that in order to decrease Moser order $m_k(\M A)$ to 0 (and, thus, decreasing Poincar\'e rank to zero), we may need to repeat Step~2 up to $n \times m_k(\M A)$ times, which in the case of large $n$ may be a challenging computational problem.
...
\begin{enumerate}
  \item Given a matrix $\M A(x,\eps)$, find a rational transformation $\M T(x,\eps)$ such that the equivalent matrix $\M A'(x,\eps)$ is Fuchsian.
  \item Given a Fuchsian matrix $\M A(x,\eps)$ with eigenvalues of all its residues in  $n+m\eps$ form, where $n$ is integer, find a rational transformation $\M T(x,\eps)$ such that the equivalent matrix $\M A'(x,\eps)$ is Fuchsian and normalized.
  \item Given a Fuchsian and normalized matrix $\M A(x,\eps)$, find a rational transformation $\M T(x,\eps)$ such that the equivalent matrix is canonical, i.e., $\M A'(x,\eps) = \eps \, \M A'(x)$.
\end{enumerate}
...
By {\em fuchsification} we mean to find a rational transformation $\M T(x,\eps)$ which transforms a matrix $\M A(x,\eps)$, at all singular points $x=x_k$ including $\infty$, to the Fuchsian form given by a matrix $\M A'(x,\eps)$.

The strategy is exactly the same as the recursive criterion for calculating $\mu_k(\M A)$ discussed above, but now our aim is to find $\M T(x,\eps)$ rather than $\mu_k(\M A)$.
...
If however $\mu_k(\M A)$ will turn out not to be $0$, we'll know that it's impossible to reduce $\M A(x,\eps)$ to Fuchsian form, and we'll have no choice but to abandon further reduction.
...
Otherwise, we proceed with constructing an appropriate transformation, which plays a crucial role in the Lee method and is referred as a {\em $\M P$-balance} between $x_1$ and $x_2$, given by
\begin{equation}
\label{eq:bal}
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
where $\M P(\eps)$ is an especially crafted projector matrix.
For the complete description of how to build an appropriate projector $\M P(\eps)$ see~\cite[p.~9]{Lee15}.

Let us comment on some properties of the transformation in eq.~\eqref{eq:bal}.
\begin{itemize}
  \item When appropriately constructed the $\M P$-balance decreases $\rank(\M A_{k0})$ at $x=x_k$ and does not increase Poincar'e rank at any other point.
  \item It may happen, that it is not possible to construct an appropriate $\M P$-balance between $x=x_0$ and any other existing singular point $x=x_k$.
