...
In \fuchsia, normalization step is implemented by function \code{normalize}.
See Section~\ref{sec:usage_py} for it's usage.
...
In \fuchsia the process of shuffling a matrix to it's shortest lower block-diagonal form is performed by \code{block\_triangular\_form} routine; fuchsification and normalization of diagonal blocks is done by \code{reduce\_diagonal\_blocks}; fuchsification of the remaining off-diagonal blocks is done by \code{fuchsify\_off\_diagonal\_blocks}.
...
Transform matrix $\M M$ into a lower block-triangular form.

Return three values: transformed matrix $\M M'$, transformation matrix $\M T$, and a list of tuples $(m_i, n_i)$, where $n_i$ is the size of $i$-th diagonal block, and $m_i = \sum_{j=1}^{i-1} n_j$.
The tuple list represents block structure of $\M M'$; it is used by the next two functions, where it is passed as argument $B$.

\functionitem{reduce\_diagonal\_blocks}{\M M, x, epsilon, B=None, seed=0}
Find a transformation that reduces diagonal blocks of $\M M$ into epsilon form.
If $B$ is not provided, transform $\M M$ into lower triangual form before reduction, otherwise assume $\M M$ blocks are described by $B$.

Return two values: transformed matrix $\M M$ and transformation matrix $\M T$.

\functionitem{fuchsify\_off\_diagonal\_blocks}{\M M, x, epsilon, r=None}
Given a matrix $M$ with diagonal blocks in epsilon form, transform off-diagonal blocks in to Fuchsian form.
If $B$ is not provided, transform $\M M$ into lower triangual form before reduction, otherwise assume $\M M$ blocks are described by $B$.
