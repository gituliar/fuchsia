...
Note that we've used construct \texttt{(A - B).is\_zero()} to compare matrices instead of the more obvious \texttt{bool(A == B)}.
...
Transform a system defined by matrix $\M M$ and independent variable $x$ using transformation matrix $\M T$ as specified by eq.~\eqref{eq:bta}.
...
Return matrix residue at infinity if $x_0$ is $Infinity$.

\functionitem{export\_matrix\_to\_file}{filename, \M M, fmt=\texttt{"mtx"}}
Write matrix $\M M$ to a file $filename$ using MatrixMarket array format if $fmt$ is \texttt{"mtx"} (which is the default), or Mathematica format if $fmt$ is \texttt{"m"}.
