...
\begin{abstract}
We present \fuchsia\xspace --- implementation of the Lee algorithm, which for a given system of ordinary differential equations with rational coefficients $\partial_x\,\V f(x,\eps) = \M A(x,\eps)\,\V f(x,\eps)$ finds a basis transformation $\M T(x,\eps)$, i.e., $\V f(x,\eps) = \M T(x,\eps)\,\V g(x,\eps)$, such that a given system turns into the epsilon form $\partial_x\, \V g(x,\eps) = \eps\,\M S(x)\,\V g(x,\eps)$.
...
We start by reducing {\bf diagonal blocks} of this matrix, $\M A^{(ab)}(x,\eps)$, to canonical form.
...
Finally, we factorize the whole matrix as described in Section~\ref{sec:fact}, thus completing transformation to canonical form.
